...
HOT_BOOSTER_MODULES = {"hot_region_loop", "fetch_amplifier"}
COUNT_METRICS = [
    "cpu-cycles:u",
    "instructions:u",
    "br_retired:u",
...
def is_hot_booster_case(case):
    ...
    return module_name in HOT_BOOSTER_MODULES
...
def apply_suite_defaults(cfg, suite_cfg):
    memory_defaults = suite_cfg.get("memory_defaults", {})
    if memory_defaults:
        memory_cfg = cfg.setdefault("memory", {})
        memory_cfg.update(memory_defaults)
    run_defaults = suite_cfg.get("run_defaults", {})
    if run_defaults:
        cfg.setdefault("run", {}).update(run_defaults)
    return cfg


def build_single_cfg(base_cfg, case, suite_cfg=None):
    cfg = deepcopy(base_cfg)
    if suite_cfg:
        apply_suite_defaults(cfg, suite_cfg)
...
def build_combined_cfg(base_cfg, selected_cases, ordered_cases, suite_cfg=None):
    cfg = deepcopy(base_cfg)
    if suite_cfg:
        apply_suite_defaults(cfg, suite_cfg)
    zero_all_modules(cfg)
...
def choose_random_case_sets(case_groups, combo_size, sample_count, rng, combo_filter=None):
    ...
    while len(selected) < sample_count and attempts < max_attempts:
        ...
        if cases and all(is_hot_booster_case(case) for case in cases):
            continue
        if combo_filter is not None and not combo_filter(cases):
            continue
...
def run_random_combo_suite(base_cfg, knobs, metric_keys, suite_cfg, output_dir):
    ...
    for combo_size in combo_sizes:
        case_sets = choose_random_case_sets(
            ...
            rng,
            combo_filter=suite_cfg.get("combo_filter"),
        )
        for combo_index, selected_cases in enumerate(case_sets):
            ...
            for case in selected_cases:
                row, raw_output = write_tagged_run(
                    build_single_cfg(base_cfg, case, suite_cfg),
                    knobs,
                    csv_headers,
                    csv_out,
                    suite_name=suite_name,
            ...
            for ordered_cases, order_tag in ordered_runs:
                cfg = build_combined_cfg(base_cfg, selected_cases, ordered_cases, suite_cfg)
                row, raw_output = write_tagged_run(
                    cfg,
                    knobs,
                    csv_headers,
